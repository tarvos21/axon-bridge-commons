...
export class AxonApiHandler implements API.ForceBridgeAPIV1 {
  ...
  constructor(axonRpcUrl: string) {
    ...
    this.crossChainContract = new ethers.Contract(
      this.crossChainAddress,
      CrossChain.abi,
      this.provider
    );
    this.wCkbAddress = "0x4af5ec5e3d29d9ddd7f4bf91a022131c41b72352";
    this.wCkbContract = new ethers.Contract(
      this.wCkbAddress,
      ERC20.abi,
      this.provider
    );
  ...
  getMinimalBridgeAmount(
    payload: API.GetMinimalBridgeAmountPayload
  ): Promise<API.GetMinimalBridgeAmountResponse> {
    ...
    return (async () => {
      ...
      const erc20_amount = await this.crossChainContract.fee(
        payload.xchainAssetIdent,
        payload.amount
      );
  ...
  getBridgeInNervosBridgeFee(
    ...
    return (async () => {
      const fee_amount = await this.crossChainContract.fee(
        payload.xchainAssetIdent,
        payload.amount
      );
      const fee = fee_amount.toString();
      console.log(
        `getBridgeInNervosBridgeFee: ${JSON.stringify(payload)}, ${fee}`
      );
      return { fee };
    })();
  }
  ...
  async generateBridgeInNervosTransaction<T extends NetworkTypes>(
    ...
    // eslint-disable-next-line no-console
    console.log(
      `generateBridgeInNervosTransaction, payload: ${JSON.stringify(payload)}`
    );

    // check allowance of wCKB
    let allowance = await this.wCkbContract.allowance(
      payload.sender,
      this.crossChainAddress
    );
    if (allowance != ethers.constants.MaxUint256) {
      let result = await this.wCkbContract.approve(
        this.crossChainAddress,
      ...
      case "Ethereum": {
        ...
        switch (payload.asset.ident) {
          case "0x0000000000000000000000000000000000000000":
            tx = await this.crossChainContract.populateTransaction.lockAT(
              payload?.recipient,
              {
          ...
          default:
            ...
            tx =
              await this.crossChainContract.populateTransaction.crossTokenToCKB(
                recipient,
                payload.asset.ident,
                transferAmount
              );
    ...
    return Promise.resolve([
      {
        ...
        info: {
          ...
          shadow: {
            network: "Nervos",
            ident:
              "0xb3fdbe0ddaf0b8d3e427fd56968a1eaefa7b57acb36cce34bafebeae22e0ebef",
          },
        },
      },
      // {
  ...
  async getBalance(
    payload: API.GetBalancePayload
  ): Promise<API.GetBalanceResponse> {
    ...
    return (await Promise.all(
      balanceFutures
    )) as unknown as Promise<GetBalanceResponse>;
  }
  ...
  async getBridgeConfig(): Promise<API.GetConfigResponse> {
    ...
    return Promise.resolve({
      nervos: {
        ...
        confirmNumber: 15,
        omniLockCodeHash:
          "0xea5d73e46455979b498e7c6eb4eb88af285ad474c3a7eab98d16e0d9210d56f1",
